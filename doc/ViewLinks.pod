...
=head1 SYNOPSIS
    ...
    fieldhashes \my (%OWNER, %NEXT, %LAST, %CURRENT);
    ...
    sub get_owner   { my ($view)  = @_; my $i = $OWNER{$view};    defined $i ? id_2obj($i) : undef }
    sub get_next    { my ($view)  = @_; my $i = $NEXT{$view};     defined $i ? id_2obj($i) : undef }
    sub get_last    { my ($group) = @_; my $i = $LAST{$group};    defined $i ? id_2obj($i) : undef }
    sub get_current { my ($group) = @_; my $i = $CURRENT{$group}; defined $i ? id_2obj($i) : undef }

    # --- Ringlisten-Konvention wie Turbo Vision: first == last->next ---
    sub get_first {
        my ($group) = @_;
        my $last = get_last($group) or return undef;     # leere Gruppe
    ...
    sub children {
        ...
        my (@out, $v) = ((), $first);
        do { push @out, $v; $v = get_next($v) } while defined($v) && $v != $first;
        return @out;
    }
...
in Perl I<referenzfrei> abgebildet werden kann, ohne dabei die
...
=item *
C<current> - aktiver View (Fokus/Selektion)

=back

Statt Perl-Referenzen werden nur noch C<IDs> (numerische Schlüssel)
gespeichert. Die Auflösung der ID vom Objekt erfolgt über die
...
=item *
Threadsichere ID in Objekt-Rückauflösung (ggf. für zukünftige Versionen)
=back
...
=item * C<%OWNER>:    $OWNER{$view}    = id($group)

=item * C<%NEXT>:     $NEXT{$view}     = id($next_view)

=item * C<%LAST>:     $LAST{$group}    = id($last_child)

=item * C<%CURRENT>:  $CURRENT{$group} = id($current_child)
...
=item *
Hinweis: Bereits beim Erzeugen eines Wrappers sollte das Obkect registriert 
werden (z.B. im Konstruktor), damit ID-Rückauflösung immer verfügbar ist.
...
=head2 Setzen & Lesen
    ...
    set_owner($view,  $group)
    set_next($view,   $next)
    set_last($group,  $last)
    set_current($group, $cur)

    get_owner($view)      -> $group|undef
    get_next($view)       -> $view|undef
    get_last($group)      -> $view|undef
    get_current($group)   -> $view|undef
    get_first($group)     -> $view|undef   # synonym: get_next(get_last)
...
Beispiel-Implementierungen:
    ...
    sub insert_after {
        ...
        my $next = get_next($target) // $target;     # Einzel-View?
        set_next($new_view, $next);
        set_next($target,   $new_view);
        set_owner($new_view, $group);
        if ($target == get_last($group)) {
            set_last($group, $new_view);
        }
    }
    ...
    sub insert_first {
        ...
        if (my $last = get_last($group)) {
            ...
            set_next($new_view, $new_view);  # Ring mit 1 Element
            set_owner($new_view, $group);
            set_last($group, $new_view);
        }
    }
    ...
    sub remove_view {
        my ($group, $view) = @_;
        my $last = get_last($group) or return;
        my $pred;

        my $v = get_next($last);
        do {
            if (get_next($v) == $view) { $pred = $v; last }
            $v = get_next($v);
        } while $v != get_next($last);
        ...
        if ($view == $last) {
            ($view_next == $view) ? set_last($group, undef)   # war einziges View
                                  : set_last($group, $pred);
        }
...
=item *
Alle Views einer C<$group> bilden eine kreisförmige einfach verkettete Liste
über C<next>; die Z-Reihenfolge entspricht der TVision-View-Reihenfolge.

=back

=head1 THREADING

Falls Threading jemals ein Thema wird so ist eine Nutzung mit 
FieldHash-Registry (C<register>, C<id>, C<id_2obj>) threadsicher:
...
zu dieser ID gibt). Um es strikter zu machen, müssten die Getter bei 
...
(z.B. im Konstruktor), sonst liefert C<id_2obj> C<undef> zurück.
...
=item 2
Neue FieldHashes C<%OWNER>, C<%NEXT> ... anlegen.

=item 3
Beim Link setzen C<id($obj)> statt der Referenz speichern.

=item 4
An Lese-Callsites C<id_2obj($id)> einführen (gekapselt in C<get_owner()>, ...).
...
=item *
Threads (falls doch genutzt): in einem Worker-Thread Ring aufbauen, im
Hauptthread lesen.
...
berücksichtigt GC/Threading. Reines C<refaddr> könnte wiederverwendet werden, 
wenn Objekte freigegeben wurden; ich will es daher vermeide.
